...
public final class AppConfig {
    ...
    /**
     * [CA] Nom de l'aplicació, llegit de {@code app.properties} (generat per
     * Maven a partir de {@code project.name} del {@code pom.xml} mitjançant
     * filtratge de recursos, vegeu {@code <build><resources>} al pom) perquè
     * no calgui actualitzar-lo manualment a cada canvi de versió. Aquest
     * recurs es genera tant compilant des de NetBeans/IDE com empaquetant el
     * JAR. Si per algun motiu no es troba, retorna un valor per defecte.
     * <p>
     * [EN] Application name, read from {@code app.properties} (generated by
     * Maven from the pom.xml {@code project.name} via resource filtering, see
     * {@code <build><resources>} in the pom) so it never needs manual updating
     * on a version bump. This resource is generated both when compiling from
     * NetBeans/IDE and when packaging the JAR. Falls back to a default if it
     * cannot be found for any reason.
     *
     * @return [CA] nom de l'aplicació / [EN] application name
     */
    public static String getAppName() {
        Properties appProps = new Properties();
        try (InputStream in = AppConfig.class.getResourceAsStream(APP_PROPERTIES_RESOURCE)) {
            if (in != null) {
                appProps.load(in);
            }
        ...
        String name = appProps.getProperty("app.name");
        return (name != null && !name.isBlank()) ? name : "DoDeReBu";
    }
